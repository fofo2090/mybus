...
class DatabaseService {
  ...
  // Delete student (hard delete - completely removes from database)
    ...
    try {
      ...
      for (var linkDoc in linksQuery.docs) {
        ...
        if (studentIds.isEmpty) {
          ...
          // إذا كان هناك طلاب آخرين، احذف هذا الطالب فقط من القائمة
  ...
  Future<List<StudentModel>> getStudentsByRouteSimple(String busRoute) async {
    try {
      final bus = await getBusByRoute(busRoute);
      if (bus != null) {
        return await getStudentsByBus(bus.id).first;
      }
      return [];
    } catch (e) {
      debugPrint('❌ Error getting students by route simple: $e');
      return [];
    }
  }
  ...
  Future<List<StudentModel>> getStudentsByBusIdSimple(String busId) async {
    try {
      return await getStudentsByBus(busId).first;
    } catch (e) {
      debugPrint('❌ Error getting students by bus id simple: $e');
      return [];
    }
  }
  ...
  Future<void> fixExistingNotifications() async {
    ...
    // The original implementation is missing.
    debugPrint('fixExistingNotifications called, but no implementation is available.');
    return;
  }
  ...
  Future<String> createSupervisorEvaluationSurvey({
    ...
    try {
      final survey = SurveyModel(
        id: '', // will be generated by createSurvey
        title: 'تقييم المشرف: $supervisorName',
        description: 'يرجى تقييم أداء المشرف $supervisorName.',
        type: SurveyType.supervisorEvaluation,
        questions: [
          SurveyQuestion(id: 'q1', question: 'ما مدى رضاك عن أداء المشرف بشكل عام؟', type: QuestionType.rating, options: ['1', '2', '3', '4', '5'], order: 1, isRequired: true),
          SurveyQuestion(id: 'q2', question: 'هل لديك أي ملاحظات إضافية؟', type: QuestionType.text, order: 2, isRequired: false),
        ],
        createdBy: parentId,
      ...
      throw Exception('فشل في إنشاء استبيان تقييم المشرف');
    }
  }

  Future<List<AbsenceModel>> getTodayAbsencesForSupervisorSimple(String supervisorId) async {
    try {
      return await getTodayAbsencesForSupervisor(supervisorId).first;
    } catch (e) {
      debugPrint('❌ Error getting today absences for supervisor simple: $e');
      return [];
    }
  }

  Future<List<String>> getSupervisorStudentIds(String supervisorId) async {
      final List<String> studentIds = [];
      final assignments = await getSupervisorAssignmentsSimple(supervisorId);
      for (final assignment in assignments) {
        final students = await getStudentsByBusIdSimple(assignment.busId);
        for (final student in students) {
  ...
  Future<List<AbsenceModel>> getAbsencesInDateRangeSimple(String supervisorId, DateTime startDate, DateTime endDate) async {
    try {
      final absencesSnapshot = await _firestore
          .collection('absences')
      ...
      final studentIds = await getSupervisorStudentIds(supervisorId);
      ...
      debugPrint('❌ Error getting absences in date range simple: $e');
      return [];
    }
  }
  ...
  Stream<int> getActiveStudentsCount() {
    // Stub
    return _firestore.collection('students').where('isActive', isEqualTo: true).snapshots().map((snapshot) => snapshot.docs.length);
  }

  Stream<int> getTotalParentsCount() {
    // Stub
    return _firestore.collection('users').where('userType', isEqualTo: 'parent').snapshots().map((snapshot) => snapshot.docs.length);
  }

  Stream<int> getTotalSupervisorsCount() {
    // Stub
    return _firestore.collection('users').where('userType', isEqualTo: 'supervisor').snapshots().map((snapshot) => snapshot.docs.length);
  }

  Stream<int> getTotalBusesCount() {
    // Stub
    return _firestore.collection('buses').snapshots().map((snapshot) => snapshot.docs.length);
  }

  Stream<int> getActiveTripCount() {
    // Stub
    return _firestore.collection('trips').where('status', isEqualTo: 'active').snapshots().map((snapshot) => snapshot.docs.length);
  }
  ...
  Stream<int> getTodayTripsCount() {
    ...
    final today = DateTime(now.year, now.month, now.day);
    final tomorrow = today.add(const Duration(days: 1));
    return _firestore.collection('trips').where('timestamp', isGreaterThanOrEqualTo: Timestamp.fromDate(today)).where('timestamp', isLessThan: Timestamp.fromDate(tomorrow)).snapshots().map((snapshot) => snapshot.docs.length);
  }
  ...
  Stream<List<Map<String, dynamic>>> getAllStudentsWithAbsenceData() {
    return getAllStudents().asyncMap((students) async {
      final List<Map<String, dynamic>> result = [];
      for (final student in students) {
        final absences = await getAbsencesByStudent(student.id).first;
        result.add({
